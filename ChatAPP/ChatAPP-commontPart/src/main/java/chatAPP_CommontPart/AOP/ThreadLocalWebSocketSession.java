...
public @interface ThreadLocalWebSocketSession {
	...
	@Pointcut("execution(void *.*(..)) && @annotation(ThreadLocalWebSocketSession)")
	...
	private void processItAsync(ProceedingJoinPoint joinPoint,RabitMQPropertiesAOP aop) {
		String evnokedBy=joinPoint.getClass().getName()+"."+joinPoint.getSignature().getName();
		 ...
		 this.manipulation.setSimpMessageHeaderAccessor(ses, null);
		try {
			joinPoint.proceed();
		} catch (Throwable e) {
